	 ...
	 if(new_net==1)
	 {
		...
		PartitionL:for(i=0;i<MAXNOLAYERS;i++)
		{
		 ...
		 if(conv==1)
		 {
			...
			NPartionsC=(uint8)(convOut1D/NUM_PE);
			NExtraPE_NeededC=convOut1D-(NPartionsC*NUM_PE);

	#ifndef __SYNTHESIS__
			cout<< "NPartionsC:"<< NPartionsC <<'\n';
			cout<< "NExtraPE_NeededC:"<< NExtraPE_NeededC <<'\n';
	#endif
		 }
	}
			...
			row: for(i=0;i<K;i++)
			{
				if(i<kernelSize)
				{
					col:for(j=0;j<K;j++)
					{
						if(j<kernelSize)
						{
							inCount = i*imageSize*channels+j*channels+p*(kernelSize)+n*imageSize;
							PE_input.N0_input[inputCount]= inputs[inCount];
							PE_input.N1_input[inputCount]= inputs[inCount+1];
							PE_input.N2_input[inputCount]= inputs[inCount+2];
							PE_input.N3_input[inputCount]= inputs[inCount+3];
							PE_input.N4_input[inputCount]= inputs[inCount+4];
							PE_input.N5_input[inputCount]= inputs[inCount+5];
							PE_input.N6_input[inputCount]= inputs[inCount+6];
							PE_input.N7_input[inputCount]= inputs[inCount+7];
							inputCount++;
						}
						else
							break;
				...
				outCount = p+n+count;
				PE_output.PE0_output[outCount]=PE0_Unit<fp6, fp1, fp2>(PE_input.N0_input, kernels, ksize);
				PE_output.PE1_output[outCount]=PE1_Unit<fp6, fp1, fp2>(PE_input.N1_input, kernels, ksize);
				PE_output.PE2_output[outCount]=PE2_Unit<fp6, fp1, fp2>(PE_input.N2_input, kernels, ksize);
				PE_output.PE3_output[outCount]=PE3_Unit<fp6, fp1, fp2>(PE_input.N3_input, kernels, ksize);
				PE_output.PE4_output[outCount]=PE4_Unit<fp6, fp1, fp2>(PE_input.N4_input, kernels, ksize);
				PE_output.PE5_output[outCount]=PE5_Unit<fp6, fp1, fp2>(PE_input.N5_input, kernels, ksize);
				PE_output.PE6_output[outCount]=PE6_Unit<fp6, fp1, fp2>(PE_input.N6_input, kernels, ksize);
				PE_output.PE7_output[outCount]=PE7_Unit<fp6, fp1, fp2>(PE_input.N7_input, kernels, ksize);
...
//	cout<<"outIndex " << p+n+count <<'\n';
	copy1:for(m=0;m<MaxConvOut1D;m++)
	{
		...
		copy2:for(q=0;q<NPc;q++)
		{
			...
			{
#ifndef __SYNTHESIS__
					cout<< "PE_output.PE0_output:"<< PE_output.PE0_output[m+q+count2] <<'\n';
#endif
				inputs[count1]   = PE_output.PE0_output[m+q+count2];
				inputs[count1+1] = PE_output.PE1_output[m+q+count2];
				inputs[count1+2] = PE_output.PE2_output[m+q+count2];
				inputs[count1+3] = PE_output.PE3_output[m+q+count2];
				inputs[count1+4] = PE_output.PE4_output[m+q+count2];
				inputs[count1+5] = PE_output.PE5_output[m+q+count2];
				inputs[count1+6] = PE_output.PE6_output[m+q+count2];
				inputs[count1+7] = PE_output.PE7_output[m+q+count2];
				count1 = count1+8;
			}
			else
				break;
