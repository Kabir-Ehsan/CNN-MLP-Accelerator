...
int get_label(volatile float *Output, int n);
...
/*	configData.word_0 = NLAYERS;
	...
	volatile static float dout[OUTPUTS];
	volatile static float dout2[OUTPUTS];
	...
	{      //inputs
		//commented//
	   ////din[i]=float_to_fixed(0.1);//InputPatterens[row][i];
	   din[i]= input_arr[i];
	   printf("din[%d]= %f  \n\r", i, din[i]);
	   //din[i]=InputPatterens[row][i];
		//din2[i]=InputPatterens[19][i];
    }

	for(i=0;i<MaxW;i++)
	{
	  //commented//
	   //////din[i+INPUTS]=float_to_fixed(0.2);//weight_Ann[i]
	   //printf("din[%d]= %d  \n\r", i, din[i]);
	   din[i+INPUTS]=weights[i];
	}

	printf("din[%d]= %f  \n\r", i, din[i+INPUTS-1]);

	 unsigned int indma_size = (MaxW+INPUTS) * sizeof(float);
	 unsigned int indma_size2 = (INPUTS) * sizeof(float);
	 unsigned int outdma_size = OUTPUTS * sizeof(float);
	...
	/* unsigned int indma_size = (MaxW+INPUTS) * sizeof(u32);
	 ...
	 //-----------------------------------------------------------------------
		...
		if(status != XST_SUCCESS)
		{
			...
			XController_Set_Config_imageSize_V(&Controller, IMAGE);
			XController_Set_Config_KernelSize_V(&Controller, kernel);
			XController_Set_Config_channels_V(&Controller, channels);
			XController_Set_Config_stride_V(&Controller, stride);
			XController_Set_Config_padding_V(&Controller, padding);
			XController_Set_Config_KSIZE_V(&Controller, KSIZE);
			XController_Set_Config_convOut1D_V(&Controller, CONVOUT1D);
...
/*
			...
			status = XAxiDma_SimpleTransfer(&axiDma, (u32)din, indma_size, XAXIDMA_DMA_TO_DEVICE);
			status = XAxiDma_SimpleTransfer(&axiDma, (u32)dout, outdma_size, XAXIDMA_DEVICE_TO_DMA);
			...
			if (status != XST_SUCCESS)
			{
				...
				//begin_time1 =XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR, XPAR_TMRCTR_0_DEVICE_ID);
				//while (XAxiDma_Busy(&axiDma, XAXIDMA_DMA_TO_DEVICE)||XAxiDma_Busy(&axiDma,  XAXIDMA_DEVICE_TO_DMA));
			while (XAxiDma_Busy(&axiDma,  XAXIDMA_DEVICE_TO_DMA));
				//end_time1 = XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR,XPAR_TMRCTR_0_DEVICE_ID);
			...
			end_time4 = XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR,XPAR_TMRCTR_0_DEVICE_ID);
				...
				//begin_time2 =XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR,XPAR_TMRCTR_0_DEVICE_ID);

				/*if (status != XST_SUCCESS)
				{
					print("Error: Vivado HLS block to DMA transfer failed\n");
					return XST_FAILURE;
				}
				//while (XAxiDma_Busy(&axiDma,  XAXIDMA_DEVICE_TO_DMA)) ;
				//end_time2 = XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR,XPAR_TMRCTR_0_DEVICE_ID);

				//end_time3= XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR,XPAR_TMRCTR_0_DEVICE_ID);

				//run_time_hw1 = end_time1 - begin_time1 - calibration;
				//run_time_hw2 = end_time2 - begin_time2 - calibration;
...
*/
		...
		status = XAxiDma_SimpleTransfer(&axiDma, (UINTPTR)din, indma_size, XAXIDMA_DMA_TO_DEVICE);
		...
		status = XAxiDma_SimpleTransfer(&axiDma, (UINTPTR) dout,outdma_size, XAXIDMA_DEVICE_TO_DMA);
		if (status != XST_SUCCESS) {
